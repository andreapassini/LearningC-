...
Scalar rad2deg(Scalar k){
    return k/(M_PI)*180;
}
Scalar deg2rad(Scalar k){
    return k*(M_PI)/180;
}

Vector3 mix(Vector3 a, Vector3 b, Scalar t){
    return a*(1-t) + b*t;
}

Scalar mix(Scalar a, Scalar b, Scalar t){
    return a*(1-t) + b*t;
}



Scalar angleBetween( const Vector3 &a , const Vector3 &b ) {
